/* -----------------------------------------------------------------------------
**	PERMEDIA2 PLL meta language binding
** -----------------------------------------------------------------------------
**	Copyright	1999-2000	Steffen Seeger
**
**	This file is distributed under the terms and conditions of the 
**	MIT/X public license. Please see the file COPYRIGHT.MIT included
**	with this software for details of these terms and conditions.
**
** -----------------------------------------------------------------------------
**	MAINTAINER	Steffen_Seeger
**
**	$Log: PERMEDIA2-bind.h,v $
**	Revision 1.1.1.1  2000/04/18 08:51:13  aldot
**	import of kgi-0.9 20020321
**	
**	Revision 1.1.1.1  2000/04/18 08:51:13  seeger_s
**	- initial import of pre-SourceForge tree
**	
*/
#ifndef _clock_3Dlabs_PERMEDIA2_bind_h
#define	_clock_3Dlabs_PERMEDIA2_bind_h

#include "clock/3Dlabs/PERMEDIA2-meta.h"

KGIM_META_INIT_MODULE_FN(pgc_clock)
KGIM_META(pgc_clock)

#endif	/* #ifndef _clock_3Dlabs_PERMEDIA2_bind_h	*/
