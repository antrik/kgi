...
static int board_probe(device_t dev);
static int board_attach(device_t dev);
static int board_detach(device_t dev);
...
unsigned int monitor_ddc = 2;	MODULE_PARM(monitor_ddc, "0-2i");
...
static inline void kgim_options_init(kgim_display_t *dpy,
		...
		kgim_monitor_init_module_fn *monitor_init)
{
	kgi_u_t i;
	struct {
...
static devclass_t model##_devclass;					\
									\
static device_method_t model##_methods[] = {				\
	DEVMETHOD(device_probe,		board_probe),			\
	DEVMETHOD(device_attach,	board_attach),			\
	DEVMETHOD(device_detach,	board_detach),			\
									\
	{ 0, 0 }							\
};									\
									\
static driver_t model##_driver = {					\
	#model,								\
	model##_methods,						\
	sizeof(struct board_softc),					\
};									\
									\
DRIVER_MODULE(board_##model, pci, model##_driver, model##_devclass, 0, 0);\
									\
MODULE_DEPEND(board_##model, monitor_##monitor, KGIM_MINVER, KGIM_PREFVER, KGIM_MAXVER);\
MODULE_DEPEND(board_##model, kgim, KGIM_MINVER, KGIM_PREFVER, KGIM_MAXVER);\
MODULE_VERSION(board_##model, 1);					\
									\
...
#define	Board(vendor,model,monitor)					\
static int								\
board_probe(device_t dev)						\
{									\
	const kgi_u32_t *subsystemID =					\
		 kgim_##vendor##_##model##_subsystemID;			\
									\
	while (*subsystemID) {						\
		if (PCICFG_SIGNATURE(pci_get_vendor(dev),		\
				     pci_get_device(dev)) ==		\
		    *subsystemID) 					\
				    break;				\
		else subsystemID++;					\
...
#define	Model(model_name)						\
	device_set_desc(dev, "KGI " model_name " driver");

#define	Driver(sys, driver, meta)

#define End								\
	pcicfg_add_device(dev);						\
	return 0;							\
}
