#!/usr/bin/awk -f
# -----------------------------------------------------------------------------
#	AWK script to convert VESA timing file into monosync timing C code
# -----------------------------------------------------------------------------
...
#
#	command-line usage:	
#		vesa2c.awk timingset=<file-base-name> <file-name>
#
#	$Log: vesa2c.awk,v $
...
#	if (timings) {
		...
		for (i = 0; i < timings; i++) {
			...
			printf("\t{\n");
			printf("\t\t\"%i x %i @ %iHz\",\n",
			        width[i], height[i], refresh[i]);
			printf("/* %.3f */\t{ %6i, %6i, %6i, %6i, %6i, %6i, CRT_%s },\n",
				1e6/xtotal[i],
				xaddr[i], xbstart[i], xsstart[i],
				xsstart[i]+xstime[i], xbstart[i]+xbtime[i],
				xtotal[i], hsync[i]);
			printf("/* %.1f */\t{ %6i, %6i, %6i, %6i, %6i, %6i, CRT_%s },\n",
				1e9/xtotal[i]/ytotal[i],
				yaddr[i], ybstart[i], ysstart[i],
				ysstart[i]+ystime[i], ybstart[i]+ybtime[i],
				ytotal[i], vsync[i]);
			printf("/* dclk */\t0\n");
			printf((i < timings-1) ? "\t},\n" : "\t}\n");
		}
